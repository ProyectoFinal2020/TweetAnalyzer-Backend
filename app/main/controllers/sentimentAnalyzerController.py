...
sentimentBucket = SentimentAnalyzerDto.sentimentBucket
sentimentBucketWithTweets = SentimentAnalyzerDto.sentimentBucketWithTweets
...
class SentimentAnalyzerController(Resource):
    ...
    def get(self):
        ...
        if(not reportId or not algorithm or not threshold):
            ...
            return sentimentAnalyzer.getSentimentsFilteredByPolarityValueAndThreshold(topic_title=topicTitle, \
                min_polarity=min_polarity, max_polarity=max_polarity, page=page, per_page=per_page,\
                report_id=reportId, algorithm=algorithm, threshold=threshold)   
...
class SentimentAnalyzerDownloadController(Resource):
    ...
    @api.marshal_list_with(sentimentBucketWithTweets)
    def get(self):
        """
        Returns a list of polarity buckets filled with tweets generated by a similarity algorithm.
        ...
        if(not reportId or not algorithm or not threshold):
            ...
            return sentimentAnalyzer.getTweetCountForPolarityBucketsFilteredBySimAlgorithm(report_id=reportId, \
                topic_title=topicTitle, algorithm=algorithm, threshold=threshold, step_size=step_size, includeTweets=True)     
...
class SentimentAnalyzerGraphController(Resource):
    ...
    @api.marshal_list_with(sentimentBucket)
    def get(self):
        """
        Returns an array of tweet numbers according to polarity buckets filtering by similarity algorithm.
